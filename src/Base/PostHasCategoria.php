...
use \Categoria as ChildCategoria;
use \CategoriaQuery as ChildCategoriaQuery;
use \Post as ChildPost;
use \PostHasCategoriaQuery as ChildPostHasCategoriaQuery;
use \PostQuery as ChildPostQuery;
use \Exception;
use \PDO;
use Map\PostHasCategoriaTableMap;
...
abstract class PostHasCategoria implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PostHasCategoriaTableMap';
    ...
    /**
     * Initializes internal state of Base\PostHasCategoria object.
     */
    ...
    /**
     * Compares this with another <code>PostHasCategoria</code> instance.  If
     * <code>obj</code> is an instance of <code>PostHasCategoria</code>, delegates to
     * <code>equals(PostHasCategoria)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|PostHasCategoria The current object, for fluid interface
     */
     ...
     * @return $this|\PostHasCategoria The current object (for fluent API support)
     */
    public function setPostId($v)
    {
        ...
        if ($this->post_id !== $v) {
            $this->post_id = $v;
            $this->modifiedColumns[PostHasCategoriaTableMap::COL_POST_ID] = true;
        }
    ...
    public function setCategoriaId($v)
    {
        ...
        if ($this->categoria_id !== $v) {
            $this->categoria_id = $v;
            $this->modifiedColumns[PostHasCategoriaTableMap::COL_CATEGORIA_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PostHasCategoriaTableMap::translateFieldName('PostId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->post_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PostHasCategoriaTableMap::translateFieldName('CategoriaId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = PostHasCategoriaTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\PostHasCategoria'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildPostHasCategoriaQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see PostHasCategoria::setDeleted()
     * @see PostHasCategoria::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildPostHasCategoriaQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                PostHasCategoriaTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PostHasCategoriaTableMap::COL_POST_ID)) {
            $modifiedColumns[':p' . $index++]  = 'post_id';
        }
        if ($this->isColumnModified(PostHasCategoriaTableMap::COL_CATEGORIA_ID)) {
            $modifiedColumns[':p' . $index++]  = 'categoria_id';
        }

        $sql = sprintf(
            'INSERT INTO post_has_categoria (%s) VALUES (%s)',
    ...
    {
        $pos = PostHasCategoriaTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['PostHasCategoria'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['PostHasCategoria'][$this->hashCode()] = true;
        $keys = PostHasCategoriaTableMap::getFieldNames($keyType);
        $result = array(
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PostHasCategoriaTableMap::COL_POST_ID)) {
            $criteria->add(PostHasCategoriaTableMap::COL_POST_ID, $this->post_id);
        }
        if ($this->isColumnModified(PostHasCategoriaTableMap::COL_CATEGORIA_ID)) {
            $criteria->add(PostHasCategoriaTableMap::COL_CATEGORIA_ID, $this->categoria_id);
        }
    ...
    {
        $criteria = ChildPostHasCategoriaQuery::create();
        $criteria->add(PostHasCategoriaTableMap::COL_POST_ID, $this->post_id);
        $criteria->add(PostHasCategoriaTableMap::COL_CATEGORIA_ID, $this->categoria_id);
    ...
    {
        return (null === $this->getPostId()) && (null === $this->getCategoriaId());
    }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PostHasCategoria (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PostHasCategoria Clone of current object.
     * @throws PropelException
     */
    ...
    public function getPost(ConnectionInterface $con = null)
    {
        if ($this->aPost === null && ($this->post_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                $this->aPost->addPostHasCategorias($this);
             */
        }
    ...
    public function getCategoria(ConnectionInterface $con = null)
    {
        if ($this->aCategoria === null && ($this->categoria_id !== null)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aCategoria->addPostHasCategorias($this);
             */
        }
    ...
    {
        if (null !== $this->aPost) {
            $this->aPost->removePostHasCategoria($this);
        }
        if (null !== $this->aCategoria) {
            $this->aCategoria->removePostHasCategoria($this);
        }
        $this->post_id = null;
    ...
    {
        return (string) $this->exportTo(PostHasCategoriaTableMap::DEFAULT_STRING_FORMAT);
    }
