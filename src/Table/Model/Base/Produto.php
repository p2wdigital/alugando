...
abstract class Produto implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Table\\Model\\Map\\ProdutoTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildProdutoValor[] Collection to store aggregation of ChildProdutoValor objects.
     */
    protected $collProdutoValors;
    protected $collProdutoValorsPartial;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildProdutoValor[]
     */
    protected $produtoValorsScheduledForDeletion = null;

    /**
     * Initializes internal state of Table\Model\Base\Produto object.
     */
    ...
    public function setCategoria($v)
    {
        ...
        if ($this->categoria !== $v) {
            $this->categoria = $v;
            $this->modifiedColumns[ProdutoTableMap::COL_CATEGORIA] = true;
        }
    ...
    public function setNome($v)
    {
        ...
        if ($this->nome !== $v) {
            $this->nome = $v;
            $this->modifiedColumns[ProdutoTableMap::COL_NOME] = true;
        }
    ...
    public function setModelo($v)
    {
        ...
        if ($this->modelo !== $v) {
            $this->modelo = $v;
            $this->modifiedColumns[ProdutoTableMap::COL_MODELO] = true;
        }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->produtoValorsScheduledForDeletion !== null) {
                if (!$this->produtoValorsScheduledForDeletion->isEmpty()) {
                    \Table\Model\ProdutoValorQuery::create()
                        ->filterByPrimaryKeys($this->produtoValorsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->produtoValorsScheduledForDeletion = null;
                }
            }

            if ($this->collProdutoValors !== null) {
                foreach ($this->collProdutoValors as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ProdutoTableMap::COL_CATEGORIA)) {
            $modifiedColumns[':p' . $index++]  = 'categoria';
        }
        if ($this->isColumnModified(ProdutoTableMap::COL_NOME)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Table\Model\Produto
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Table\Model\Produto The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ProdutoTableMap::COL_CATEGORIA)) {
            $criteria->add(ProdutoTableMap::COL_CATEGORIA, $this->categoria);
        }
        if ($this->isColumnModified(ProdutoTableMap::COL_NOME)) {
            $criteria->add(ProdutoTableMap::COL_NOME, $this->nome);
        }
        if ($this->isColumnModified(ProdutoTableMap::COL_MODELO)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Table\Model\Produto (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getProdutoValors() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addProdutoValor($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Table\Model\Produto Clone of current object.
     * @throws PropelException
     */
    ...
    public function setOrcamentoItems(Collection $orcamentoItems, ConnectionInterface $con = null)
    {
        ...
        foreach ($orcamentoItemsToDelete as $orcamentoItemRemoved) {
            $orcamentoItemRemoved->setProduto(null);
        }
     ...
     * @return ObjectCollection|ChildOrcamentoItem[] List of ChildOrcamentoItem objects
     */
    public function getOrcamentoItemsJoinOrcamento(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildOrcamentoItemQuery::create(null, $criteria);
        $query->joinWith('Orcamento', $joinBehavior);

        return $this->getOrcamentoItems($query, $con);
    }

    /**
     * Clears out the collProdutoValors collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return void
     * @see        addProdutoValors()
     */
    public function clearProdutoValors()
    {
        $this->collProdutoValors = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collProdutoValors collection loaded partially.
     */
    public function resetPartialProdutoValors($v = true)
    {
        $this->collProdutoValorsPartial = $v;
    }

    /**
     * Initializes the collProdutoValors collection.
     *
     * By default this just sets the collProdutoValors collection to an empty array (like clearcollProdutoValors());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initProdutoValors($overrideExisting = true)
    {
        if (null !== $this->collProdutoValors && !$overrideExisting) {
            return;
        }
        $this->collProdutoValors = new ObjectCollection();
        $this->collProdutoValors->setModel('\Table\Model\ProdutoValor');
    }

    /**
     * Gets an array of ChildProdutoValor objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildProdutoValor[] List of ChildProdutoValor objects
     * @throws PropelException
     */
    public function getProdutoValors(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collProdutoValorsPartial && !$this->isNew();
        if (null === $this->collProdutoValors || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collProdutoValors) {
                // return empty collection
                $this->initProdutoValors();
            } else {
                $collProdutoValors = ChildProdutoValorQuery::create(null, $criteria)
                    ->filterByProduto($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collProdutoValorsPartial && count($collProdutoValors)) {
                        $this->initProdutoValors(false);

                        foreach ($collProdutoValors as $obj) {
                            if (false == $this->collProdutoValors->contains($obj)) {
                                $this->collProdutoValors->append($obj);
                            }
                        }

                        $this->collProdutoValorsPartial = true;
                    }

                    return $collProdutoValors;
                }

                if ($partial && $this->collProdutoValors) {
                    foreach ($this->collProdutoValors as $obj) {
                        if ($obj->isNew()) {
                            $collProdutoValors[] = $obj;
                        }
                    }
                }

                $this->collProdutoValors = $collProdutoValors;
                $this->collProdutoValorsPartial = false;
            }
        }

        return $this->collProdutoValors;
    }

    /**
     * Sets a collection of ChildProdutoValor objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $produtoValors A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildProduto The current object (for fluent API support)
     */
    public function setProdutoValors(Collection $produtoValors, ConnectionInterface $con = null)
    {
        ...
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->produtoValorsScheduledForDeletion = clone $produtoValorsToDelete;

        foreach ($produtoValorsToDelete as $produtoValorRemoved) {
            $produtoValorRemoved->setProduto(null);
        }

        $this->collProdutoValors = null;
        foreach ($produtoValors as $produtoValor) {
            $this->addProdutoValor($produtoValor);
        }

        $this->collProdutoValors = $produtoValors;
        $this->collProdutoValorsPartial = false;
    ...
    /**
     * Returns the number of related ProdutoValor objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related ProdutoValor objects.
     * @throws PropelException
     */
    public function countProdutoValors(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collProdutoValorsPartial && !$this->isNew();
        if (null === $this->collProdutoValors || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collProdutoValors) {
            ...
            if ($partial && !$criteria) {
                return count($this->getProdutoValors());
            }

            $query = ChildProdutoValorQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collProdutoValors);
    }

    /**
     * Method called to associate a ChildProdutoValor object to this object
     * through the ChildProdutoValor foreign key attribute.
     *
     * @param  ChildProdutoValor $l ChildProdutoValor
     * @return $this|\Table\Model\Produto The current object (for fluent API support)
     */
    public function addProdutoValor(ChildProdutoValor $l)
    {
        if ($this->collProdutoValors === null) {
            $this->initProdutoValors();
            $this->collProdutoValorsPartial = true;
        ...
        if (!$this->collProdutoValors->contains($l)) {
            $this->doAddProdutoValor($l);
        }
    ...
    /**
     * @param ChildProdutoValor $produtoValor The ChildProdutoValor object to add.
     */
    protected function doAddProdutoValor(ChildProdutoValor $produtoValor)
    {
        $this->collProdutoValors[]= $produtoValor;
        $produtoValor->setProduto($this);
    }

    /**
     * @param  ChildProdutoValor $produtoValor The ChildProdutoValor object to remove.
     * @return $this|ChildProduto The current object (for fluent API support)
     */
    public function removeProdutoValor(ChildProdutoValor $produtoValor)
    {
        if ($this->getProdutoValors()->contains($produtoValor)) {
            $pos = $this->collProdutoValors->search($produtoValor);
            $this->collProdutoValors->remove($pos);
            if (null === $this->produtoValorsScheduledForDeletion) {
                $this->produtoValorsScheduledForDeletion = clone $this->collProdutoValors;
                $this->produtoValorsScheduledForDeletion->clear();
            }
            $this->produtoValorsScheduledForDeletion[]= clone $produtoValor;
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            ...
            if ($this->collProdutoValors) {
                foreach ($this->collProdutoValors as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)
