 ...
 *
 * @method     \Table\Model\AutorQuery|\Table\Model\PostDataQuery|\Table\Model\PostHasCategoriaQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class PostQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related \Table\Model\Autor object
     *
     * @param \Table\Model\Autor|ObjectCollection $autor The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($autor instanceof \Table\Model\Autor) {
            return $this
            ...
            throw new PropelException('filterByAutor() only accepts arguments of type \Table\Model\Autor or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \Table\Model\AutorQuery A secondary query class using the current class as primary query
     */
    public function useAutorQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinAutor($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Autor', '\Table\Model\AutorQuery');
    }
